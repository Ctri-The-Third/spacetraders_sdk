...
def test_systems_view_one(system_response_data):
    ...
    symbols = [w["symbol"] for w in system_response_data["waypoints"]]
    sys: System
    assert sys
    assert sys.symbol == test_system.symbol
    assert sys.waypoints
    assert sys.waypoints[0].symbol in symbols
...
def test_ship(ship_response_data):
    client = SpaceTradersPostgresClient(
        ST_HOST, ST_NAME, ST_USER, ST_PASS, TEST_AGENT_NAME, db_port=ST_PORT
    )
    test_ship = st_models.Ship.from_json(ship_response_data)
    client.update(test_ship)
    loaded_ship = client.ships_view_one(test_ship.name)

    assert loaded_ship
    assert isinstance(loaded_ship, st_models.Ship)
    assert loaded_ship.frame.condition == test_ship.frame.condition
    assert loaded_ship.frame.integrity == test_ship.frame.integrity
    assert loaded_ship.frame.description == test_ship.frame.description
    assert loaded_ship.nav.origin.symbol == test_ship.nav.origin.symbol
    assert loaded_ship.nav.destination.symbol == test_ship.nav.destination.symbol
    assert loaded_ship.fuel_capacity == test_ship.fuel_capacity
    assert loaded_ship.nav.status == test_ship.nav.status
    assert loaded_ship.nav.flight_mode == test_ship.nav.flight_mode
...
@pytest.fixture
def ship_response_data():
    return {
        "symbol": "CTRI-W--1",
        "nav": {
            "systemSymbol": "X1-QV57",
            "waypointSymbol": "X1-QV57-A1",
            "route": {
                "departure": {
                    "symbol": "X1-QV57-A1",
                    "type": "PLANET",
                ...
                "arrival": "2024-03-10T18:15:52.933Z",
                "departureTime": "2024-03-10T18:15:52.933Z",
            },
            "status": "DOCKED",
            "flightMode": "CRUISE",
        },
        ...
        "fuel": {
            "current": 400,
            "capacity": 400,
            "consumed": {"amount": 0, "timestamp": "2024-03-10T18:15:52.933Z"},
        },
        "cooldown": {
            "shipSymbol": "CTRI-W--1",
            "totalSeconds": 0,
