...
def _upsert_shipyard(shipyard: Shipyard):
    if len(shipyard.ships) > 0:
        ...
        resp = try_execute_upsert(sql, (shipyard.waypoint, ship_type, ship_cost))
        if not resp:
            return resp
        ...
        ON CONFLICT (shipyard_symbol, ship_type) DO NOTHING;"""
            for ship_type in shipyard.ship_types:
                resp = try_execute_upsert(sql, (shipyard.waypoint, ship_type))
                if not resp:
                    return resp
